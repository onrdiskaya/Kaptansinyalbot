"""
Kesişim Radar - "Capitano Master Radar v5.5 (Log-Pivot Fibonacci Edition)"
- LOGARİTMİK FİBONACCİ PİVOT HESAPLAMA MOTORU (Log-space Pivots)
- LOGARİTMİK GRAFİK VE PİVOT ÇİZİMİ (yscale='log')
- EN AZ 3 İNDİKATÖR ONAYI ŞARTI (Confirmations Counter)
- TEYİT EDİLEN (🎯) ve TEYİT EDİLMEYEN/ADAY (⚠️) SİNYAL AYRIMI
- MUM FORMASYONLARI ANALİZİ (Yutan Boğa/Ayı, Çekiç, Doji vb.)
- RSI POZİTİF / NEGATİF UYUMSUZLUK (DIVERGENCE) TESPİTİ
- TELEGRAM KOMUT YÖNETİMİ (/add, /remove, /list, /all, /manual)
"""
...
MIN_CONFIRMATIONS = 3  # En az 3 İndikatör Onayı Şartı
MIN_CONFIDENCE_TO_NOTIFY = 40
...
def log_fibonacci_pivots(h, l, c):
    """Logaritmik Ölçekli Fibonacci Pivot Seviyeleri Hesaplaması"""
    ...
    log_pp = (log_h + log_l + log_c) / 3.0
    log_range = log_h - log_l
    
    return [
        ("R3", math.exp(log_pp + (log_range * 1.000))),
        ("R2", math.exp(log_pp + (log_range * 0.618))),
        ("R1", math.exp(log_pp + (log_range * 0.382))),
        ("PP", math.exp(log_pp)),
        ("S1", math.exp(log_pp - (log_range * 0.382))),
        ("S2", math.exp(log_pp - (log_range * 0.618))),
        ("S3", math.exp(log_pp - (log_range * 1.000)))
    ]

def generate_chart(symbol, timeframe, candles, prev_daily, filename="temp_chart.png"):
    """Logaritmik Ölçekli Grafik ve Log-Fibonacci Pivot Çizimi"""
    try:
        df_data = []
        for c in candles[-70:]:
        ...
        if prev_daily:
            pivots = dict(log_fibonacci_pivots(prev_daily["high"], prev_daily["low"], prev_daily["close"]))
            ...
            pivot_title_str = f" | Log-Fibo PP:{fmt_price(pp_v)} R1:{fmt_price(r1_v)} S1:{fmt_price(s1_v)}"
        ...
        chart_title = f"{symbol.replace('-', '')} ({timeframe}) [LOG-FIBO]{pivot_title_str}"
...
def fetch_klines_okx(symbol, timeframe, limit=250):
    ...
    try:
        ...
        if data.get("code") == "0" and data.get("data"): raw_list = data["data"]
    except Exception: pass
    if not raw_list:
        try:
            data = http_get_json(f"/api/v5/market/candles?instId={symbol}-SWAP&bar={timeframe}&limit={limit}")
            if data.get("code") == "0" and data.get("data"): raw_list = data["data"]
        except Exception as e: raise ValueError(f"Veri çekilemedi ({symbol}): {e}")
    if not raw_list: raise ValueError(f"Boş veri döndü ({symbol})")
    raw_list.reverse()
    return [{
...
def fetch_open_interest_okx(symbol):
    try:
        ...
        if data.get("code") == "0" and data.get("data"): return float(data["data"][0]["oiCcy"])
    except Exception: pass
    return 0.0
        ...
        if data.get("code") == "0" and data.get("data"):
            for item in data["data"]:
                for detail in item.get("details", []):
                    ...
                    if side == "sell": long_liq += sz
                    elif side == "buy": short_liq += sz
...
def detect_candlestick_pattern(candles):
    if len(candles) < 3: return "Nötr / Belirgin Formasyon Yok ⚪", 0, "neutral"
    ...
    if c2_range == 0: return "Nötr / Belirgin Formasyon Yok ⚪", 0, "neutral"
    
    if c1_c < c1_o and c2_c > c2_o and c2_o <= c1_c and c2_c >= c1_o and c2_body > c1_body:
...
def generate_signal(symbol, timeframe, candles, prev_daily, btc_state, funding_rate, current_oi, long_liq, short_liq, state):
    ...
    report_data = {
        ...
        "Log-Fibo Pivot": {"detail": "Pivot verisi eksik", "score": 0, "status": "neutral"},
    ...
    if vol_ratio >= 1.7:
        if is_bullish_vol: report_data["Hacim Durumu"] = {"detail": f"Güçlü ALIM Hacmi Girişi (x{vol_ratio:.1f}) 🟢", "score": 25, "status": "bullish"}
        else: report_data["Hacim Durumu"] = {"detail": f"Sert SATIŞ Hacmi / Mal Boşaltma (x{vol_ratio:.1f}) 🩸", "score": -25, "status": "bearish"}
    elif vol_ratio <= 0.6: report_data["Hacim Durumu"] = {"detail": f"Hacim Çok Kurudu (x{vol_ratio:.1f}) 🏜️", "score": -5, "status": "bearish"}
    ...
    if ema50 and ema200:
        e50_l, e200_l = ema50[-1], ema200[-1]
        e50_p, e200_p = ema50[-2], ema200[-2]
        if e50_p <= e200_p and e50_l > e200_l: report_data["EMA Yapısı"] = {"detail": "Golden Cross Kesişimi Uçtu! 🚀", "score": 45, "status": "bullish"}
        elif e50_p >= e200_p and e50_l < e200_l: report_data["EMA Yapısı"] = {"detail": "Death Cross Kesişimi Düştü! 💀", "score": -45, "status": "bearish"}
        elif price > e50_l and price > e200_l: report_data["EMA Yapısı"] = {"detail": f"Fiyat Trend Üstü Akümüle (EMA50: {fmt_price(e50_l)})", "score": 20, "status": "bullish"}
        elif price < e50_l and price < e200_l: report_data["EMA Yapısı"] = {"detail": "Fiyat EMA 50 & 200 Altında Sıkışık", "score": -20, "status": "bearish"}
        
    # --- LOGARİTMİK FİBONACCİ PİVOT ANALİZİ ---
    if prev_daily:
        pivots = log_fibonacci_pivots(prev_daily["high"], prev_daily["low"], prev_daily["close"])
        if pivots:
            ...
            if price > pp_level:
                report_data["Log-Fibo Pivot"] = {"detail": f"Log-PP Üzerinde Güçlü ({fmt_price(pp_level)}). Yakın: {nearest_name} (%{nearest_dist:.2f})", "score": 15, "status": "bullish"}
            else:
                report_data["Log-Fibo Pivot"] = {"detail": f"Log-PP Altında Baskılı ({fmt_price(pp_level)}). Yakın: {nearest_name} (%{nearest_dist:.2f})", "score": -15, "status": "bearish"}
        
    rsi_vals = calc_rsi(closes)
    if rsi_vals:
        curr_rsi, prev_rsi = rsi_vals[-1], rsi_vals[-2]
        div_status = detect_rsi_divergence(candles, rsi_vals)
        if div_status == "BULLISH_DIV": report_data["RSI Göstergesi"] = {"detail": f"⚠️ POZİTİF UYUMSUZLUK (Dipten Dönüş! RSI: {curr_rsi:.1f}) 🔥", "score": 40, "status": "bullish"}
        elif div_status == "BEARISH_DIV": report_data["RSI Göstergesi"] = {"detail": f"⚠️ NEGATİF UYUMSUZLUK (Tepeden Düşüş! RSI: {curr_rsi:.1f}) 🩸", "score": -40, "status": "bearish"}
        elif curr_rsi <= 25: report_data["RSI Göstergesi"] = {"detail": f"Dip Seviyede! Aşırı Satım (%{curr_rsi:.1f}) 🚨", "score": 35, "status": "bullish"}
    ...
    if funding_rate != 0.0:
        if funding_rate >= 0.0002: report_data["Fonlama Oranı"] = {"detail": f"Long Baskısı / Piyasada Isınma Var (%{funding_rate*100:.4f}) ⚠️", "score": -10, "status": "bearish"}
        elif funding_rate <= -0.00015: report_data["Fonlama Oranı"] = {"detail": f"Short Baskısı / Squeeze Potansiyeli! (%{funding_rate*100:.4f}) 🚀", "score": 20, "status": "bullish"}
        
    if short_liq > 0 or long_liq > 0:
        if short_liq > long_liq: report_data["Likidasyon Analizi"] = {"detail": f"Yoğun Short Patlaması Var ({short_liq:.1f} Kontrat) 🚀", "score": 15, "status": "bullish"}
        elif long_liq > short_liq: report_data["Likidasyon Analizi"] = {"detail": f"Sert Long Patlaması / Flush Gerçekleşti ({long_liq:.1f} Kontrat) 🩸", "score": -15, "status": "bearish"}
    ...
    # --- ONAY SAYACI ---
    bullish_confirmations = sum(1 for v in report_data.values() if v["status"] == "bullish")
    bearish_confirmations = sum(1 for v in report_data.values() if v["status"] == "bearish")
    
    if raw_score > 0:
        active_confirmations = bullish_confirmations
        direction = "AL"
    else:
        active_confirmations = bearish_confirmations
        direction = "SAT"

    confidence = min(100, abs(raw_score))
    is_confirmed = active_confirmations >= MIN_CONFIRMATIONS
    
    if confidence < MIN_CONFIDENCE_TO_NOTIFY and not is_confirmed:
        return None
    
    return {
        "symbol": symbol, "timeframe": timeframe, "direction": direction, 
        "confidence": confidence, "price": price, "report_data": report_data, 
        "funding": funding_rate, "confirmations": active_confirmations, "is_confirmed": is_confirmed
    }

def format_signal_message(sig):
    if sig["is_confirmed"]:
        status_tag = f"🎯 <b>TEYİT EDİLEN SİNYAL</b> ({sig['confirmations']}/9 Onay)"
        emoji = "🟢 [BOĞA RADARI - TEYİTLİ]" if sig["direction"] == "AL" else "🔴 [AYI RADARI - TEYİTLİ]"
    else:
        status_tag = f"⚠️ <b>TEYİT EDİLMEYEN / ADAY SİNYAL</b> ({sig['confirmations']}/9 Onay)"
        emoji = "🟡 [ADAY RADAR - İZLEME]"

    clean_sym = sig['symbol'].replace("-", "")
    tf_str = sig['timeframe'].replace("15m", "15 Dk").replace("1H", "1 Saat").replace("4H", "4 Saat").replace("1Dutc", "1 Gün")
    tv_link = f"https://www.tradingview.com/chart/?symbol=OKX:{clean_sym}"
    
    lines = [
        f"{emoji} <b>#{clean_sym}</b>",
        f"<b>Durum:</b> {status_tag}",
...
def single_scan(state, watchlist):
    ...
    print(f"🔄 Çok Boyutlu Log-Pivot Röntgen Taraması: {len(symbols)} sembol taranıyor...")
