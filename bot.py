"""
Kesişim Radar - "Capitano Master Radar v5.3 (Log Scale Edition)"
...
- LOGARİTMİK GRAFİK ÇİZİMİ & TELEGRAM GÖRSEL GÖNDERİMİ
"""
...
def generate_chart(symbol, timeframe, candles, prev_daily, filename="temp_chart.png"):
    """Düzeltilmiş Başlık Hizalama, Hacim, EMA, RSI ve MACD Grafiği Çizer (Logaritmik Ölçekli)."""
    try:
        df_data = []
        for c in candles[-70:]:
        ...
        chart_title = f"{symbol.replace('-', '')} ({timeframe}) [LOG]{pivot_title_str}"
